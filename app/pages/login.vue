...
const submit = async () => {
  ...
  try {
    ...
    error.value = err?.message || err?.data?.message || t('auth.loginError')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="text-center mb-[var(--spacing-xl)]">
    ...
    <h1 class="heading-md text-[var(--color-ink)]">{{ $t('auth.login') }}</h1>
    <p class="body-sm text-[var(--color-steel)] mt-1">{{ $t('auth.loginButton') }}</p>
  </div>
  ...
  <div class="mt-[var(--spacing-xl)] text-center body-sm text-[var(--color-steel)]">
    {{ $t('auth.noAccount') }}
    <NuxtLink :to="localePath('/register')" class="text-[var(--color-brand-blue-deep)] hover:underline">{{ $t('auth.goRegister') }}</NuxtLink>
  </div>
</template>
