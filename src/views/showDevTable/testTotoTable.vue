    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.billType === '收入' ? 'success' : (scope.row.billType === '支出'?'danger':'primary')"
            disable-transitions
          >{{ scope.row.billType }}
          </el-tag>
        </template>
      ...
      <el-table-column v-if="showReviewer" label="备注" width="120px" align="center">
        <template slot-scope="{row}">
...
<script>
import { fetchTotoDatabaseList } from '@/api/devDataQuery'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchTotoDatabaseList(this.listQuery).then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['timestamp', 'goods', 'payment', 'billType', 'status']
        const filterVal = ['timestamp', 'goods', 'payment', 'billType', 'status']
